...
class TeamScore(Base):
    ...
    team_key: Mapped[str] = mapped_column(
        ForeignKey("teams.team_number"), primary_key=True
    )
    event_key: Mapped[str] = mapped_column(
        ForeignKey("frcevent.event_key"), primary_key=True
    )
    ...
    event_finished: Mapped[bool] = mapped_column(
    ...
    def update_elim_points(
        self,
        lost_match_12=False,
        lost_match_13=False,
        lost_finals: bool = False,
        won_finals: bool = False,
    ):
        points = 0
        if lost_match_12:
            points = 7
...
class FantasyTeam(Base):
    ...
    league_id: Mapped[int] = mapped_column(
        ForeignKey("league.league_id"), nullable=False
    )

    waiver_priority = relationship(
        "WaiverPriority", back_populates="fantasy_team", uselist=False
    )
...
class TeamOwned(Base):
    ...
    fantasy_team_id: Mapped[int] = mapped_column(
        ForeignKey("fantasyteam.fantasy_team_id"), primary_key=True
    )
    league_id: Mapped[int] = mapped_column(
        ForeignKey("league.league_id"), primary_key=True
    )
    draft_id: Mapped[int] = mapped_column(
        ForeignKey("draft.draft_id"), primary_key=True
    )
...
class TeamStarted(Base):
    ...
    team_number: Mapped[str] = mapped_column(
        ForeignKey("teams.team_number"), primary_key=True
    )
    league_id: Mapped[int] = mapped_column(
        ForeignKey("league.league_id"), primary_key=True
    ...
    week: Mapped[int] = mapped_column(
        Integer(), nullable=False, default=0, primary_key=True
    )
...
class PlayerAuthorized(Base):
    ...
    player_id: Mapped[str] = mapped_column(
        ForeignKey("players.user_id"), primary_key=True
    )
    fantasy_team_id: Mapped[int] = mapped_column(
        ForeignKey("fantasyteam.fantasy_team_id"), primary_key=True
    )
