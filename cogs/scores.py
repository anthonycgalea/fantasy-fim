...
from models.scores import FRCEvent, League, Team, TeamOwned, TeamScore, TeamStarted
...
class Scores(commands.Cog):
    ...
    async def getFRCTeamReport(self, interaction: discord.Interaction, frcTeam: str):
        async with self.bot.async_session() as session:
            ...
            stmt = select(Team).where(Team.team_number == frcTeam, Team.is_fim)
            result = await session.execute(stmt)
            ...
            if not leagueToGrabReport:
                ...
                team_status = (
                    "Free Agent"  # Change to "On Waivers" or "Free Agent" as needed
                )

                stmt = select(TeamOwned).where(
                    TeamOwned.league_id == leagueToGrabReport.league_id,
                ...
                if ownedBy:
                    ...
                    stmt = select(TeamOnWaivers).where(
                        TeamOnWaivers.league_id == leagueToGrabReport.league_id,
                        TeamOnWaivers.team_number == frcTeam,
                    )
                    result = await session.execute(stmt)
                ...
                stmt = (
                    ...
                    .where(
                        ...
                        TeamStarted.league_id == leagueToGrabReport.league_id,
                    )
                )
                result = await session.execute(stmt)
                ...
                stmt = (
                    ...
                    .where(
                        TeamScore.team_key == frcTeam,
                        FRCEvent.year == leagueToGrabReport.year,
                    )
                ...
                # Create the embed
                embed = discord.Embed(
                    title=f"**Team {team_number} Report**", color=discord.Color.blue()
                )
                ...
                embed.add_field(
                    name="Team Starts", value=str(team_starts), inline=False
                )
                ...
                events_info = "\n".join(
                    [
                        (
                            f"{score.event.event_name} - {score.score_team()} points"
                            if score.score_team() > 0
                            else f"{score.event.event_name} - Week {score.event.week}"
                        )
                        for score in events_attending
                        if score.event.is_fim
                    ]
                )
                embed.add_field(
                    name="Events Attending",
                    value=events_info if events_info else "None",
                    inline=False,
                )
    ...
    @app_commands.command(
        name="getrankings", description="Retrieves rankings for the channel's league"
    )
    ...
    @app_commands.command(
        name="teamreport",
        description="Retrieves a report of a specific FRC team in the channel's league",
    )
    async def getTeamReport(self, interaction: discord.Interaction, frcteam: str):
        await interaction.response.send_message(
            f"Retrieving team report for team {frcteam}", ephemeral=True
        )
        await self.getFRCTeamReport(interaction, frcteam)

    @app_commands.command(
        name="weeklyreport",
        description="Retrieves a report of scores and rankings up to date for the current week in the channel's league",
    )
    async def getWeekReport(self, interaction: discord.Interaction):
        await interaction.response.send_message(
            "Retrieving report up to the most recent week"
        )
