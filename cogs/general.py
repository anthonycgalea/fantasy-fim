...
class General(commands.Cog):
    ...
    @app_commands.command(
        name="leagues", description="Reports on active leagues and their league ids."
    )
    async def getLeagues(self, interaction: discord.Interaction):
        async with self.bot.async_session() as session:
            stmt = select(League).where(League.active)
            ...
            for league in leagues:
                embed.description += (
                    f"{league.league_name:>15s}   <#{league.discord_channel}>\n"
                )
            await interaction.response.send_message(embed=embed)

    @app_commands.command(
        name="teams",
        description="Reports on teams in the channel's league and their team IDs.",
    )
        ...
        async with self.bot.async_session() as session:
            ...
            if league is None:
                await interaction.response.send_message(
                    "No league associated with this channel"
                )
                return
            leagueid = league.league_id
            draftOrderEmbed = Embed(
                title=f"**Teams in {league.league_name}**",
                description=f"```{'Team ID':7s}{'':5s}{'Team Name (id)':30s}{'Waiver':^6s}\n",
            )
            stmt = (
                select(FantasyTeam)
            ...
            for team in fantasyTeams:
                if team.waiver_priority is None:
    ...
    async def waiverPriorityReport(self, interaction: discord.Interaction):
        async with self.bot.async_session() as session:
            ...
            stmt = (
                ...
                .where(WaiverPriority.league_id == leagueid)
    ...
    @app_commands.command(
        name="leaguesite", description="Retrieve a link to your league's webpage"
    )
    ...
    @app_commands.command(
        name="draftsite", description="Retrieve a link to your draft's webpage"
    )
    async def getDraftWebpage(self, interaction: discord.Interaction):
        async with self.bot.async_session() as session:
            ...
            if draft is None:
                await interaction.response.send_message(
                    "No draft associated with this channel"
                )
                return
    ...
    @app_commands.command(
        name="website", description="Retrieve a link to the fantasy FiM website"
    )
    async def getWebsite(self, interaction: discord.Interaction):
        await interaction.response.send_message(f"{websiteURL}")

    @app_commands.command(
        name="api", description="sends a link to the swagger page for the API"
    )
    ...
    @app_commands.command(
        name="weekstatus",
        description="Reports on the status of the current fantasy FiM week",
    )
    async def getWeekStatus(self, interaction: discord.Interaction):
        currentWeek: WeekStatus = await self.bot.getCurrentWeek()
        embed = Embed(
            title=f"**Current Week: Week {currentWeek.week} of {currentWeek.year}**",
            description="",
        )
    ...
    @app_commands.command(
        name="standings",
        description="Reports on the rankings for the league in this channel",
    )
    async def getLeagueStandingsTask(self, interaction: discord.Interaction, week: int):
        await interaction.response.send_message(
            f"Retrieving standings as of week {week}"
        )
        async with self.bot.async_session() as session:
            stmt = select(League).where(
                League.is_fim,
            ...
            if league:
                ...
                await interaction.channel.send(
                    content="No league associated with this channel!"
                )

    @app_commands.command(
        name="randomize", description="Randomly pick a team from a comma-separated list"
    )
    ...
    @app_commands.command(
        name="joindraft", description="Join an offseason draft! Can specify a team name"
    )
    async def joinOffseasonDraft(
        self, interaction: discord.Interaction, teamname: str = None
    ):
        async with self.bot.async_session() as session:
            ...
            if not league.offseason:
                await interaction.response.send_message(
                    "This league is not an offseason league."
                )
                return
            ...
            if player_authorization:
                await interaction.response.send_message(
                    "You are already part of a fantasy team in this league."
                )
                return
            ...
            if draft_started:
                await interaction.response.send_message(
                    "The draft for this league has already started."
                )
                return
            ...
            if teamname:
                stmt = select(FantasyTeam).where(
                    FantasyTeam.league_id == league.league_id,
                    FantasyTeam.fantasy_team_name == teamname,
                )
                result = await session.execute(stmt)
            ...
            player_authorized = PlayerAuthorized(
                player_id=str(interaction.user.id), fantasy_team_id=new_team_id
            )
            ...
            await interaction.response.send_message(
                f"Successfully joined the offseason draft with team '{new_team_name}' and team ID {new_fantasy_team.fantasy_team_id}!"
            )
