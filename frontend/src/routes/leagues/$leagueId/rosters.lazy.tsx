...
export const RostersPage = () => {
	...
	if (rosterWeeks.isLoading || rosters.isLoading || fantasyTeams.isLoading || league.isLoading) {
		return <div>Loading...</div>;
	}

	const handleChange = (val: string) => setSelectedTeam(parseInt(val));

	const selectedRoster = rosterWeeks.data?.find((team) => team.fantasy_team_id === selectedTeam);
			...
			<Select value={selectedTeam.toString()} onValueChange={handleChange}>
				...
				<SelectContent>
					...
					{fantasyTeams.data?.map((team) => (
						<SelectItem key={team.fantasy_team_id} value={team.fantasy_team_id.toString()}>
							{team.team_name}
						</SelectItem>
					))}
				</SelectContent>
			</Select>
			{selectedTeam === 0 ? <RostersGrid rosters={rosters.data ?? []} year={league.data?.year} /> : <RosterWeeksTable rosterWeek={selectedRoster} />}
		</div>
	);
};

const RostersGrid = ({ rosters, year }: { rosters: { fantasy_team_id: number; fantasy_team_name: string; roster: string[] }[]; year: number | undefined }) => {
	return (
		<Table>
			<TableHeader>
				<TableRow>
